...
to-report free [ road-patches ] ; turtle procedure
  let this-car self
  report road-patches with [ pxcor < 0 and
...
to luces
  ask cars
  [ ask patches in-cone (1 + speed) 180
      [ set pcolor red ] ]
...
to go
   ;luces
   ifelse block-lane? [blockers][ask blocks [die] set t 0]
  create-or-remove-cars
  ask cars [ move-forward ]
  ask cars with [ patience <= 0 and  blocking-block = nobody] [ choose-new-lane ]
  ask cars with [ patience <= 0 and  blocking-block != nobody] [ set target-lane  ycor - 2  ]
  ask cars with [ ycor != target-lane and  blocking-block = nobody] [ move-to-target-lane ]
  ask cars with [ ycor != target-lane and  blocking-block != nobody] [ move-to-target-lane1 ]
  flujo
...
;ifelse block-lane?
  ...
  let blocking-blocks other blocks in-cone (2) 180 with [ y-distance <= 4 ]
  ;print blocking-blocks
  set blocking-block min-one-of blocking-blocks [ distance myself ]
  if blocking-block != nobody [
  ...
  ifelse blocking-block != nobody [ slow-down-car] [ speed-up-car forward speed ] ; we tentatively speed up, but might have to slow down
...
to speed-up-car ; turtle procedure
  ...
  if speed > limit-speed [  set speed  limit-speed ]
end
...
to choose-new-lane ; turtle procedure
 ...
 ;ifelse blocklane? [let other-lanes remove ycor lanes
  let other-lanes []



  set other-lanes remove ycor lanes
...
to move-to-target-lane1 ; turtle procedure
  set heading ifelse-value (target-lane < ycor) [ 180 ] [ 0 ]
  let blocking-cars other cars in-cone (1 + abs (ycor - target-lane)) 180 with [ x-distance <= 1 ]
  let blocking-car min-one-of blocking-cars [ distance myself ]
  ifelse blocking-car = nobody [
...
to blockers
  ...
  if t = 0 [
    ask cars with [(xcor > 41 and pycor > 3)] [die]
    ask cars with [(xcor > 51 and pycor > 1)] [die]
    ask cars with [(xcor > 61 and pycor > -1)] [die]

  ask patches with [pxcor > 41  and pycor = 4]
    [ sprout-blocks  1 [set color  one-of [45 10]]]
ask patches with [pxcor > 51  and pycor = 2]
    [ sprout-blocks  1[set color  one-of [45 10]]]
ask patches with [pxcor > 61  and pycor = 0]
    [ sprout-blocks  1[set color  one-of [45 10]]]
...
to flujo
  ...
  let fl count cars with [ xcor > 98 and xcor < 100]


  set list_flujo lput fl list_flujo

    set list_flujo remove 0 list_flujo
    let n length list_flujo
    if n != 0 [set f mean list_flujo]


  set mean_speed mean [speed] of cars
...
to save
  ...
  let file_out_name  (word "Tráfico.txt")
 file-open file_out_name                                          ;;   q, number of agents in the bigger cultural domain (normalized), number of cultural domains (normalized)
  ;  file-print (word q " " (giant-component-size / number_of_agents) " " (number_of_cultural_regions / number_of_agents) " " time)


      file-print (word   ticks " " number-of-cars " " acceleration " " deceleration " " max-patience " " limit-speed " "  " " ticks " " precision f 2 " " precision mean_speed 2 " "  )
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot f"
...
<experiments>
  <experiment name="experiment1" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="block-lane?">
      <value value="false"/>
    </enumeratedValueSet>
    <steppedValueSet variable="max-patience" first="10" step="10" last="100"/>
    <steppedValueSet variable="acceleration" first="0.001" step="0.002" last="0.009"/>
    <steppedValueSet variable="limit-speed" first="0.5" step="0.5" last="4"/>
    <steppedValueSet variable="number-of-cars" first="50" step="50" last="250"/>
    <steppedValueSet variable="deceleration" first="0.001" step="0.002" last="0.009"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
